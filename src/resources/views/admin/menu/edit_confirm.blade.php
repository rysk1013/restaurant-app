                    ...
                    <div class="bg-white py-6 sm:py-8 lg:py-12">
                        <div class="max-w-screen-2xl px-4 md:px-8 mx-auto">
                            ...
                            <div class="mb-10 md:mb-16">
                                <h2 class="text-gray-800 text-2xl lg:text-3xl font-bold text-center mb-4 md:mb-6">編集（確認）</h2>
                            </div>
                            <!-- text - end -->

                            <!-- form - start -->
                            <form action="{{ route('menu.update', ['id' => $posts['id']]) }}" method="post" class="max-w-screen-md grid sm:grid-cols-2 gap-4 mx-auto" novalidate>
                                @csrf
                                <div class="sm:col-span-2">
                                    <label for="menu_name" class="inline-block text-gray-800 text-sm sm:text-base mb-2">ID</label>
                                    <input disabled type="text" name="menu_name" value="{{ $posts['id'] }}" class="w-full bg-gray-400 text-gray-50 border focus:ring ring-indigo-300 rounded outline-none transition duration-100 px-3 py-2" />
                                    <input type="hidden" name="menu_name" value="{{ $posts['id'] }}" class="w-full text-gray-800 border focus:ring ring-indigo-300 rounded outline-none transition duration-100 px-3 py-2" />
                                </div>
                                ...
                                <div class="sm:col-span-2 flex justify-between items-center">
                                    ...
                                    <button name="send" type="submit" class="inline-block bg-green-500 hover:bg-green-600 active:bg-green-700 focus-visible:ring bg-green-300 text-white text-sm md:text-base font-semibold text-center rounded-lg outline-none transition duration-100 px-8 py-3">編集する</button>
                                </div>
                            </form>
                            <!-- form - end -->
                        </div>
